...
class ImageEditor:
    def __init__(self, args, LAVTRIS_mask, decoding_features) -> None:
        self.args = args
        self.mask = LAVTRIS_mask
        ...
        self.model, self.diffusion = create_model_and_diffusion(**self.model_config, mask_features=decoding_features)
    ...
    def edit_image_by_prompt(self):
        ...
        self.mask_pil = Image.fromarray(self.mask*255)
        ...
        mask_contours, _ = cv2.findContours(
            image_mask_pil_binarized, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        largest_contour = max(mask_contours, key=cv2.contourArea)
        convex_hull = cv2.convexHull(largest_contour)
        convex_mask = np.zeros_like(image_mask_pil_binarized)
        image_mask_pil_binarized = cv2.drawContours(convex_mask, [convex_hull], 0, 255, -1)
